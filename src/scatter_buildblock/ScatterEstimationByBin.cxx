...
post_processing()
{
    ...
    //    //
    info("Zooming activity image");

    VoxelsOnCartesianGrid<float>* activity_image_ptr = dynamic_cast<VoxelsOnCartesianGrid<float>* >(this->activity_image_sptr.get());
    VoxelsOnCartesianGrid<float>* attenuation_image_ptr = dynamic_cast<VoxelsOnCartesianGrid<float>* >(this->atten_image_sptr.get());
    ...
    VoxelsOnCartesianGrid<float>* attenuation_lowres_ptr = dynamic_cast<VoxelsOnCartesianGrid<float>* >(attenuation_lowres.get());
    ...
    if (this->scatter_sim_par_filename.size() > 0 )
    {
        ...
        this->scatter_simulation_sptr->set_density_image_and_subsample(this->atten_image_lowres_sptr);
    ...
    {
        // Applying mask
        // Should I zoom the attenuation image to the size of the activity image?????
...
process_data()
{
    ...
    // Reconstruct in full 3D, with the last scatter estimation sinogram,
    // or with all.
