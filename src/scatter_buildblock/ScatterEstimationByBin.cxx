...
initialise_keymap()
{
    ...
    this->parser.add_key("mask parameter filename",
                         &this->mask_parameters_filename);
...
post_processing()
{
    ...
    {
    shared_ptr<DiscretisedDensity<3, float> > tmp_image =
    ...
    this->set_atten_image_sptr(tmp_image);

//    this->calculate_mask_image();
    }
    ...
    if (this->recompute_atten_coeff)
    {
        ...
        this->calculate_multiplicative_projdata(this->output_proj_data_sptr,
    ...
    if (this->recompute_mask_image)
    {
        ...
        filter_ptr.reset();
        KeyParser local_parser;
        local_parser.add_start_key("PostFilteringParameters");
        local_parser.add_parsing_key("PostFilter type", &filter_ptr);
        local_parser.add_stop_key("END PostFiltering Parameters");
        local_parser.parse(this->mask_parameters_filename.c_str());

        this->mask_image_sptr.reset( dynamic_cast < DiscretisedDensity < 3, float > * >
                (this->atten_image_sptr.get()));

        filter_ptr->apply(*this->mask_image_sptr.get());
    ...
    // if input data are not 2D

    this->proj_data_info_2d_ptr =
            dynamic_cast<ProjDataInfoCylindricalNoArcCorr* >
            (SSRB(*input_projdata_sptr->get_proj_data_info_ptr(), -1));

    this->proj_data_info_2d_sptr.reset(this->proj_data_info_2d_ptr);

    if (this->input_projdata_sptr->get_max_segment_num() > 0)
    {

        std::string out_filename = this->input_projdata_filename + "_2d";
        this->input_projdata_ssrb_sptr.reset(new ProjDataInterfile(this->template_exam_info_sptr,
                                                          this->proj_data_info_2d_sptr,
                                                          out_filename,
                                                         std::ios::in | std::ios::out | std::ios::trunc));

        SSRB(*this->input_projdata_ssrb_sptr.get(),
             *this->input_projdata_sptr.get());
